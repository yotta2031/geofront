<template>
  <div class="weixin-tasks">
    <el-card shadow="hover" class="mb-4">
      <template #header><span>创建发布任务</span></template>
      <el-form :model="form" label-width="120px" style="max-width: 640px">
        ...
        <el-form-item label="自媒体账号" required>
          <el-select
            v-model="form.accountIds"
            multiple
            placeholder="仅显示已开启发布的账号"
            style="width: 100%"
          >
            <el-option
              v-for="acc in enabledAccounts"
              :key="acc.id"
              :label="`${acc.accountName}（${acc.platform}）`"
              :value="acc.id"
            />
          </el-select>
        </el-form-item>
...
import { getWeixinAccounts, getWeixinTasks, createWeixinTask } from '@/api/weixin'
...
async function fetchTasks() {
  loading.value = true
  try {
    const res = await getWeixinTasks({ page: page.value, pageSize: 20 })
    if (res.code === 1) {
...
async function createTask() {
  if (!form.name.trim() || !form.articleIds.length || !form.accountIds.length) {
    ElMessage.warning('请填写任务名称并选择文章和账号')
    return
  }
  creating.value = true
  try {
    const res = await createWeixinTask({ ...form })
    if (res.code === 1) {
...
onMounted(async () => {
  const [a, b, c] = await Promise.all([
    ...
    getWeixinAccounts({ page: 1, pageSize: 100 }),
    fetchTasks(),
  ])
