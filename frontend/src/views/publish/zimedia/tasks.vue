<template>
  <div class="zimedia-tasks">
    <el-card shadow="hover" class="mb-4">
      <template #header><span>创建大V发布任务</span></template>
      <el-form :model="form" label-width="120px" style="max-width: 640px">
        ...
        <el-form-item label="大V账号" required>
          <el-select
            v-model="form.accountIds"
            multiple
            placeholder="仅显示已开启发布的大V"
            style="width: 100%"
          >
            <el-option
              v-for="acc in enabledAccounts"
              :key="acc.id"
              :label="`${acc.accountName}（${acc.platform}·${acc.category || '其他'}）`"
              :value="acc.id"
            />
          </el-select>
        </el-form-item>
...
import { getZimediaAccounts, getZimediaTasks, createZimediaTask } from '@/api/zimedia'
...
const enabledAccounts = computed(() =>
  accounts.value.filter((a) => a.publishEnabled)
)
...
async function fetchTasks() {
  loading.value = true
  try {
    const res = await getZimediaTasks({ page: page.value, pageSize: 20 })
    if (res.code === 1) {
...
async function createTask() {
  if (!form.name.trim() || !form.articleIds.length || !form.accountIds.length) {
    ElMessage.warning('请填写任务名称并选择文章和大V账号')
    return
  }
  creating.value = true
  try {
    const res = await createZimediaTask({ ...form })
    if (res.code === 1) {
...
onMounted(async () => {
  const [a, b] = await Promise.all([
    ...
    getZimediaAccounts({ page: 1, pageSize: 100 }),
  ])
  if (a.code === 1) articles.value = a.data?.list || []
  if (b.code === 1) accounts.value = b.data?.list || []
  fetchTasks()
})
</script>
